...
Node * TreeInitializer::InitializeRandomTree(TreeInitShape init_shape, size_t max_height, const std::vector<Operator *> & functions, const std::vector<Operator *> & terminals, size_t nr_trees, bool rightheavy = false) const {
...
SingleNode* TreeInitializer::GenerateTreeGrow(size_t height_left, size_t cur_height, const std::vector<Operator*>& functions, const std::vector<Operator*>& terminals) const {
    ...
    if (height_left > 0) {
        if (rnd >= 0.5) { // || height_left > 2 ) {
            rnd = arma::randu();
            index = double(rnd) * double(functions.size());
            curr = new SingleNode(functions[index]->Clone());
        } else {
            rnd = arma::randu();
            index = rnd * (double)terminals.size();
            curr = new SingleNode((terminals[index])->Clone());
        }
