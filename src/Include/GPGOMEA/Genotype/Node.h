...
public:
    ...
    virtual void ClearCachedOutput(bool also_ancestors, bool also_children = false);

    virtual Node * CloneSubtree() const;

    virtual std::string GetSubtreeExpression(bool only_active, bool shadow = false,  bool spaces = false);

    virtual std::string GetSubtreeHumanExpression(bool shadow = false);

    virtual std::string GetPythonExpression(bool shadow = false);
    ...
    int DominatesTernary(Node * o);
    bool Dominates(Node * o);

    virtual int Count_N_NaComp(int count=-1);

    virtual std::vector<Operator *> GetNrFCConstants();

    virtual size_t GetNrSingleNodes();

    virtual Node * CloneSingleSubtree(size_t index);
