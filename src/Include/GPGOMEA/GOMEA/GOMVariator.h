...
public:
    ...
    GOMMO(Node *&sol, const std::vector<Node *> &donors, const std::vector<std::vector<size_t>> &FOS, Fitness &fit,
        bool use_caching, MOArchive *mo_archive, size_t obj, bool extreem, size_t NIS_const, bool batching);
    ...
    check_changes_SO(Node *offspring, arma::vec back_obj,size_t back_cons, bool FI, size_t obj);

    static std::pair<bool, bool>
    check_changes_MO(Node *offspring, arma::vec back_obj,size_t back_cons, bool FI, MOArchive *mo_archive, bool batching);
