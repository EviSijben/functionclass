...
void EvolutionState::ReadAndSetDataSets(po::variables_map& vm) {
    ...
    string val_path;
    if (vm.count("val")){
        val_path  = vm["val"].as<string>();
        V = fitness->ReadFitnessCases(val_path);
        fitness->SetFitnessCases(V, FitnessCasesType::FitnessCasesVALIDATION);
    }else if (vm.count("validation_perc")) {
        validation_perc = vm["validation_perc"].as<double_t>();
        ...
        fitness->SetFitnessCases(V, FitnessCasesType::FitnessCasesVALIDATION);
        //if function class and no valdiation specified, use train data
        }else if(vm.count("functionclass")) {
            V = fitness->ReadFitnessCases(train);
            fitness->SetFitnessCases(V, FitnessCasesType::FitnessCasesVALIDATION);
        }
    ...
    size_t nr_cols;
    //if function class then the first column is the id's; they are not part of the input data
    if (vm.count("functionclass")){
        nr_cols = TR.n_cols - 2;
    }else{
        nr_cols = TR.n_cols - 1;
    }
    cout << "# train: " << train << " ( " << TR.n_rows << "x" << nr_cols << " )" << endl;
    if (!V.empty() && vm.count("validation_perc"))
        cout << "# validation: " << validation_perc << " of train ( " << V.n_rows << "x" << nr_cols  << " )" << endl;
    else{
        cout << "# validation: " << val_path << " ( " << V.n_rows << "x" << nr_cols << " )" << endl;
    }
    cout << "# test: " << test << " ( " << TE.n_rows << "x" << nr_cols  << " )" << endl;
    ...
    if (vm.count("functionclass")) {
        ...
        double_t min_erc = -5 * biggest_val; //min(fitness->TrainY);
        double_t max_erc = 5 * biggest_val; //max(fitness->TrainY);
        config->terminals.push_back(new OpFunctionClassConst(min_erc, max_erc));
        config->functionclass = true;

    }else{
        if (vm.count("calcconstraints") ) {
            throw std::runtime_error("EvolutionState::constraints calculation only implemented for FCFitness");
        }
    }
}
...
Fitness * EvolutionState::FetchFitnessFunctionGivenProbName(string prob_name) {
    ...
    } else if (prob_name.compare("diversified") == 0) {
        cout << "symbolic regression diversified error";
        fitness = new DiversifiedSymbolicRegressionFitness();
    }else if (prob_name.compare("FC") == 0) {
        cout << "Function Class fitness";
        fitness = new FCFitness(config->max_num_runs);
        if (vm.count("calcconstraints")){
            dynamic_cast<FCFitness *>(fitness)->calc_constraints = true;
        }
    }else if (prob_name.compare("DiversifiedFC") == 0) {
        cout << "Diversified Function Class fitness";
        fitness = new DiversifiedFCFitness();
    } else {
...
void EvolutionState::SetOptions(int argc, char* argv[]) {
    ...
    desc.add_options()
            ...
            ("validation_perc", po::value<double_t>(), "sets the percentage of examples to take out from the training set and use for validation of best solutions in symbolic regression (default is 0)")
            ("val", po::value<string>(), "sets validation set")
            ...
            ("functionclass", "runs function class GP-GOMEA instead regular GP-GOMEA")
            ...
            ("rvevals", po::value<size_t>(), "sets the number of evaluations per real-valued gomea run (only FC)")
            ("leftheavy", "whether to have a left heavy template")
            ("calcconstraints", "whether to calculate the constraint values (only FC)")
            ("batchsize", po::value<int>(), "uses batches of the specified size instead of the whole training set (default is disabled)")
            ("addcopiedtree", "whether to check to add the copied tree in mo_archive if multi-tree")
            ("resultspath", po::value<string>(), "path to find the mo archive for which FC constants need to be calculated");
    ...
    if (vm.count("file")) {
        ...
        string filename = file.substr(slash_pos + 1, dot_pos - slash_pos - 1);
//        if (std::getenv("GOMEA_FOLDER_PATH")){
//            string abs_folder_path = std::getenv("GOMEA_FOLDER_PATH");
//            config->results_path= abs_folder_path + "/results/" + file.substr(slash_pos -3, 3 ) + "/" + filename;
//        }else{
...
//        }
    ...
    if (prob_name.compare("multiobj") == 0) {
        ...
        for (size_t f =0; f <sub_probs.size(); f++){
            cout << "<";
            sub_fitnesses.push_back(FetchFitnessFunctionGivenProbName(sub_probs[f]));
            if (dynamic_cast<FCFitness *>(sub_fitnesses[f])){
                fit_FC = true;
                if ( vm.count("rvevals")){
                    ((FCFitness *)(sub_fitnesses[f]))->SetRVevals(vm["rvevals"].as<size_t>());
                    config->rv_evals = vm["rvevals"].as<size_t>();

                }else{
                    throw std::runtime_error("EvolutionState::should define rvevals if using FC fitness");
                }
            }
            if (dynamic_cast<DiversifiedFCFitness *>(sub_fitnesses[f])  ){
                fit_div = true;
                if (!fit_FC){
                    throw std::runtime_error("EvolutionState::SetOptions should put the FC fit before diversified FC fit");
                }

            }
            if (dynamic_cast<DiversifiedSymbolicRegressionFitness *>(sub_fitnesses[f])){
                fit_div = true;
            }
            cout << ">";
        }
        ...
        if ( (vm.count("functionclass") && !fit_FC) || (!vm.count("functionclass") && fit_FC) ) {
            throw std::runtime_error("EvolutionState::SetOptions must use FC fitness when using function class constants");
        }
        if ((fit_div) && (!vm.count("nrtrees") || vm["nrtrees"].as<size_t>() <2)){
            throw std::runtime_error("EvolutionState::SetOptions should not use the diversified fitness without multitrees");
        }

        this->fitness = new MOFitness(sub_fitnesses);
    } else {
        this->fitness = FetchFitnessFunctionGivenProbName(prob_name);
        if(dynamic_cast<FCFitness *>(fitness)){
            if (!vm.count("functionclass")){
                throw std::runtime_error("EvolutionState::SetOptions must use FC fitness when using function class constants");
            }
            if ( vm.count("rvevals")){
                ((FCFitness *)fitness)->SetRVevals(vm["rvevals"].as<size_t>());
                config->rv_evals = vm["rvevals"].as<size_t>();

            }else{
                throw std::runtime_error("EvolutionState::SetOptions must set rvevals");
            }

        }
        if (vm.count("functionclass") && !dynamic_cast<FCFitness *>(fitness)){
            throw std::runtime_error("EvolutionState::SetOptions should use the functionclass constants when using FC fitness");
        }
    ...
    if (vm.count("leftheavy")){
        config->leftheavy = true;
    }

    // IMS
    if (vm.count("ims")) {

        if (prob_name.compare("multiobj")==0 && !(vm.count("gomea"))){
            throw std::runtime_error("EvolutionState::SetOptions IMS not implemented for multi-obj problems not using GOMEA");
        }
        ...
        cout << config->num_sugen_IMS << " number of sub-generations and " << config->early_stopping_IMS << " number of generations for early stopping";
        if (config->functionclass){
            config->mod_IMS = stoi(ims_params[2]);
            cout<< "and " << config->mod_IMS <<" modules" <<endl;
        }else{
            cout<<endl;
        }
    ...
    if (vm.count("gomea")) {
        ...
        if (vm.count("gomfos")) {
            ...
            } else if (fos.compare("LT_i") == 0) {
                cout << " (FOS: Linkage Tree (with idv trees))";
                config->fos_type = FOSType::FOSLinkageTreeIdv;
            }else if (fos.compare("LT_io") == 0) {
                cout << " (FOS: Linkage Tree (with idv trees))";
                config->fos_type = FOSType::FOSLinkageTreeIdvO;
        ...
        if (prob_name.compare("multiobj")==0){
            generation_handler = new GOMEAMOGenerationHandler(config, tree_initializer, fitness, semantic_library);
        }else{
    ...
    // Multitree
    if (vm.count("nrtrees")) {
        config->nrtrees = vm["nrtrees"].as<size_t>();
        cout << "# number of trees: "+ to_string(config->nrtrees) << endl;
    }
    else{
        config->nrtrees = 1;
    }

    if ((vm.count("addcopiedtree"))) {
        if(vm["nrtrees"].as<size_t>() < 2|| !vm.count("multiobj")){
            throw std::runtime_error("EvolutionState::SetOptions if not multi-tree or not multi-objective can not set add copied tree");
        }
        config->add_copied_tree = true;
    }

    if (vm.count("resultspath")) {
        config->mopath = vm["resultspath"].as<string>();
        cout << "# path to solutions: " + vm["resultspath"].as<string>() <<endl;
    }
