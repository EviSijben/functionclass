...
FOS *RV::readFOSFromFile(FILE *file) {
    char c, string[1000];
    int i, j, k;
    FOS *new_FOS;
...
FOS *RV::learnLinkageTree(double **covariance_matrix) {
    char done;
        ...
        if (problem_index == 105 || problem_index == 106) {
            ...
            for (i = 0; i < mpm_length; i++) {
                for (j = 0; j < i; j++) {
                    ...
                    else if ((mpm[i][0] - block_start) / block_size == (mpm[j][0] - block_start) / block_size)
                        S_matrix[i][j] = randomRealUniform01() + 1e8;
    ...
    while (!done) {
        ...
        while (NN_chain_length < 3) {
            NN_chain[NN_chain_length] = determineNearestNeighbour(NN_chain[NN_chain_length - 1], S_matrix,
                                                                  mpm_number_of_indices, mpm_length);
            NN_chain_length++;
        }
        ...
        while (NN_chain[NN_chain_length - 3] != NN_chain[NN_chain_length - 1]) {
            NN_chain[NN_chain_length] = determineNearestNeighbour(NN_chain[NN_chain_length - 1], S_matrix,
                                                                  mpm_number_of_indices, mpm_length);
            if (((getSimilarity(NN_chain[NN_chain_length - 1], NN_chain[NN_chain_length]) ==
                  getSimilarity(NN_chain[NN_chain_length - 1], NN_chain[NN_chain_length - 2])))
        ...
        if (r1 >= mpm_length || r0 >= mpm_length ||
            mpm_number_of_indices[r0] + mpm_number_of_indices[r1] > FOS_element_ub) {
            NN_chain_length = 1;
            NN_chain[0] = 0;
        ...
        if (r1 < mpm_length && r1 !=
                               r0) /* This test is required for exceptional cases in which the nearest-neighbor ordering has changed within the chain while merging within that chain */
        {
            ...
            new_FOS->sets[FOS_index] = (int *) Malloc(
                    (mpm_number_of_indices[r0] + mpm_number_of_indices[r1]) * sizeof(int));
            ...
            mul0 = ((double) mpm_number_of_indices[r0]) /
                   ((double) mpm_number_of_indices[r0] + mpm_number_of_indices[r1]);
            mul1 = ((double) mpm_number_of_indices[r1]) /
                   ((double) mpm_number_of_indices[r0] + mpm_number_of_indices[r1]);
            if (random_linkage_tree) {
...
double RV::getSimilarity(int a, int b) {
    if (FOS_element_ub < number_of_parameters &&
        mpm_number_of_indices[a] + mpm_number_of_indices[b] > FOS_element_ub)
        return (0);
    if (random_linkage_tree) return (1.0 - fabs(S_vector[a] - S_vector[b]));
    return (S_matrix[a][b]);
}

int RV::determineNearestNeighbour(int index, double **S_matrix, int *mpm_number_of_indices, int mpm_length) {
    int i, result;
    ...
    if (result == index)
        result++;
    for (i = 1; i < mpm_length; i++) {
        if (((getSimilarity(index, i) > getSimilarity(index, result)) ||
             ((getSimilarity(index, i) == getSimilarity(index, result)) &&
              (mpm_number_of_indices[i] < mpm_number_of_indices[result]))) && (i != index))
            result = i;
    }
...
double **RV::computeMIMatrix(double **covariance_matrix, int n) {
    int i, j;
...
int *RV::matchFOSElements(FOS *new_FOS, FOS *prev_FOS) {
    ...
    for (i = number_of_parameters; i < new_FOS->length; i++) {
        for (j = number_of_parameters; j < new_FOS->length; j++) {
            ...
            FOS_element_similarity_matrix[i - number_of_parameters][j - number_of_parameters] =
                    (int) 10000 * (2.0 * matches / (prev_FOS->set_length[i] + new_FOS->set_length[j]));
        }
    }
...
int *RV::hungarianAlgorithm(int **similarity_matrix, int dim) {
            ...
            for (y = 0; y < dim; y++) {
                if (T[y] == 0 && slack[y] == 0) {
                    if (yx[y] == -1) {
                        ...
                        if (S[yx[y]] == 0) {
                            q[wr++] = yx[y];
                            hungarianAlgorithmAddToTree(yx[y], slackx[y], S, prev, slack, slackx, lx, ly,
                                                        similarity_matrix, dim);
                        }
                    }
                }
            }
...
void RV::hungarianAlgorithmAddToTree(int x, int prevx, short *S, int *prev, int *slack, int *slackx, int *lx, int *ly,
