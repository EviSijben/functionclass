 ...
 */
int *RV::mergeSortFitness(double *objectives, double *constraints, int number_of_solutions) {
    int i, *sorted, *tosort;
 ...
 */
void
RV::RV::mergeSortFitnessWithinBounds(double *objectives, double *constraints, int *sorted, int *tosort, int p, int q) {
    int r;
 ...
 */
void
RV::RV::mergeSortFitnessMerge(double *objectives, double *constraints, int *sorted, int *tosort, int p, int r, int q) {
    int i, j, k, first;
...
void RV::setParams() {
    ...
    fix_seed = 1;
    problem_index = 23;
    number_of_parameters = FCs.size();

    lower_user_range = ((OpFunctionClassConst *) FCs[0])->lower_b;
    upper_user_range = ((OpFunctionClassConst *) FCs[0])->upper_b;


    rotation_angle = 0;
    maximum_number_of_populations = 25;
//    maximum_number_of_evaluations = 0;
    maximum_number_of_evaluations = evals;
    vtr = 1e-10;
    use_vtr = 0;
    fitness_variance_tolerance = 0;
    //maximum_number_of_seconds = 0.1* number_of_parameters;
    use_guidelines =1;
    if (use_guidelines) {
        tau = 0.35;
...
void RV::interpretCommandLine(int argc, char **argv) {
    startTimer();
 ...
 */
void RV::parseCommandLine(int argc, char **argv) {
    int index;
 ...
 */
void RV::parseOptions(int argc, char **argv, int *index) {
    double dummy;
...
void RV::parseFOSElementSize(int *index, int argc, char **argv) {
    short noError = 1;
 ...
 */
void RV::optionError(char **argv, int index) {
 ...
 */
void RV::parseParameters(int argc, char **argv, int *index) {
    int noError;
...
void RV::checkOptions() {
    ...
    if (rotation_angle > 0 && (!learn_linkage_tree && FOS_element_size > 1 && FOS_element_size != block_size &&
...
//    loadFile(problem_index, inputfilePath);
}

/**
 * Prints the settings as read from the command line.
 */
void RV::printVerboseOverview() {
    int i;
...
void RV::initializeMemory() {
    ...
    decomposed_cholesky_factors_lower_triangle = (double ****) Malloc(
            maximum_number_of_populations * sizeof(double ***));
...
void RV::initializeNewPopulationMemory(int population_index) {
    int i, j;
    ...
    objective_values_selections[population_index] = (double *) Malloc(
            selection_sizes[population_index] * sizeof(double));

    constraint_values_selections[population_index] = (double *) Malloc(
            selection_sizes[population_index] * sizeof(double));
    ...
    if (learn_linkage_tree) {
        ...
        linkage_model[population_index]->sets = (int **) Malloc(
                linkage_model[population_index]->length * sizeof(int *));
        linkage_model[population_index]->set_length = (int *) Malloc(
                linkage_model[population_index]->length * sizeof(int));
...
void RV::initializeNewPopulation() {
 ...
 */
void RV::initializeFOS(int population_index) {
    int i;
    FILE *file;
    FOS *new_FOS;
 ...
 */
void RV::initializeDistributionMultipliers(int population_index) {
    int j;
    ...
    distribution_multipliers[population_index] = (double *) Malloc(
            linkage_model[population_index]->length * sizeof(double));
 ...
 */
void RV::initializePopulationAndFitnessValues(int population_index) {
    int j, k;
    for (j = 0; j < population_sizes[population_index]; j++) {
    ...
    for (j = 0; j < population_sizes[population_index]; j++) {
        installedProblemEvaluation(problem_index, populations[population_index][j],
                                   &(objective_values[population_index][j]), &(constraint_values[population_index][j]),
...
FOS *RV::learnLinkageTreeRVGOMEA(int population_index) {
    int i;
    FOS *new_FOS;
    ...
    if (learn_linkage_tree && number_of_generations[population_index] > 0)
        inheritDistributionMultipliers(new_FOS, linkage_model[population_index],
...
void RV::inheritDistributionMultipliers(FOS *new_FOS, FOS *prev_FOS, double *multipliers) {
    int i, *permutation;
 ...
 */
void RV::computeRanksForAllPopulations() {
    int i;
 ...
 */
void RV::computeRanksForOnePopulation(int population_index) {
    int i, *sorted, rank;

    if (!populations_terminated[population_index]) {
        sorted = mergeSortFitness(objective_values[population_index], constraint_values[population_index],
 ...
 */
void RV::writeGenerationalStatisticsForOnePopulation(int population_index) {
    int j;
    char string[1000];
    ...
    for (j = 0; j < population_sizes[population_index]; j++) {
        ...
        if (betterFitness(population_objective_worst, population_constraint_worst,
                          objective_values[population_index][j], constraint_values[population_index][j])) {
        ...
        if (betterFitness(objective_values[population_index][j], constraint_values[population_index][j],
    ...
    for (j = 0; j < population_sizes[population_index]; j++) {
        population_objective_var += (objective_values[population_index][j] - population_objective_avg) *
                                    (objective_values[population_index][j] - population_objective_avg);
        population_constraint_var += (constraint_values[population_index][j] - population_constraint_avg) *
                                     (constraint_values[population_index][j] - population_constraint_avg);
    }
    ...
    sprintf(pathString, "results/statistics.dat");
    if (total_number_of_writes == 0) {
        ...
        file = fopen(pathString, "a");

    sprintf(string, "%10d %11lf %11.3lf %15.10e %13e  ", total_number_of_generations, number_of_evaluations,
            getTimer(), elitist_objective_value, elitist_constraint_value);
    fputs(string, file);
 ...
 */
void RV::writeGenerationalSolutions(short final) {
    int i, j, k;
    char string[1000];
    ...
    file_selection = NULL;
    if (final)
        sprintf(string, "results/all_populations_generation_final.dat");
    else
        sprintf(string, "results/all_populations_generation_%05d.dat", total_number_of_generations);
    file_all = fopen(string, "w");

    for (i = 0; i < number_of_populations; i++) {
        if (final)
            sprintf(string, "results/population_%05d_generation_final.dat", i);
        else
            sprintf(string, "results/population_%05d_generation_%05d.dat", i, number_of_generations[i]);
        file_population = fopen(string, "w");

        if (number_of_generations[i] > 0 && !final) {
            sprintf(string, "results/selection_%05d_generation_%05d.dat", i, number_of_generations[i] - 1);
 ...
 */
void RV::writeGenerationalSolutionsBest(short final) {
    ...
    /* Then output it */
    if (final)
        sprintf(string, "results/best_generation_final.dat");
    else
        sprintf(string, "results/best_generation_%05d.dat", c[0]);
    file = fopen(string, "w");
 ...
 */
short RV::checkTerminationCondition() {
    short allTrue;
    int i;
    ...
    if (checkSubgenerationTerminationConditions())
        return (1);
    if (checkBestFitnessTerminationCondition())
        return (1);
...
short RV::checkBestFitnessTerminationCondition() {
    if (total_number_of_generations < 4) {
        return (0);
    } else {
        if (elitist_objective_values[total_number_of_generations - 4] * 0.99 > elitist_objective_value) {
            return (0);
        } else {
            return (1);
        }
...
short RV::checkSubgenerationTerminationConditions() {
...
short RV::checkTimeLimitTerminationCondition() {
 ...
 */
short RV::checkNumberOfEvaluationsTerminationCondition() {
 ...
 */
short RV::checkVTRTerminationCondition() {
    return (use_vtr && vtr_hit_status);
}

void RV::checkAverageFitnessTerminationCondition() {
    int i, j;
    ...
    for (i = number_of_populations - 1; i >= 0; i--) {
        ...
        if (i < number_of_populations - 1 &&
            betterFitness(average_objective_values[i + 1], average_constraint_values[i + 1],
 ...
 */
void RV::determineBestSolutionInCurrentPopulations(int *population_of_best, int *index_of_best) {
    int i, j;
 ...
 */
void RV::checkFitnessVarianceTermination() {
    int i;
 ...
 * has become too small (user-defined tolerance).
 */
short RV::checkFitnessVarianceTerminationSinglePopulation(int population_index) {
    int i;
 ...
 */
void RV::checkDistributionMultiplierTerminationCondition() {
    int i, j;
    short converged;
 ...
 */
void RV::makeSelectionsForOnePopulation(int population_index) {
    int i, j, *sorted;
 ...
 * based on diversity.
 */
void RV::makeSelectionsForOnePopulationUsingDiversityOnRank0(int population_index) {
    ...
    for (i = 0; i < number_of_rank0_solutions; i++)
        nn_distances[i] = distanceEuclidean(populations[population_index][preselection_indices[i]],
                                            populations[population_index][selection_indices[number_selected_so_far -
    ...
    while (number_selected_so_far < selection_sizes[population_index]) {
        ...
        for (i = 0; i < number_of_rank0_solutions; i++) {
            value = distanceEuclidean(populations[population_index][preselection_indices[i]],
 ...
 */
void RV::makePopulation(int population_index) {
    if (populations_terminated[population_index])
        return;
 ...
 */
void RV::estimateParameters(int population_index) {
    if (!populations_terminated[population_index]) {
...
void RV::estimateParametersML(int population_index) {
    int i, j;
 ...
 */
void RV::estimateMeanVectorML(int population_index) {
    int i, j;
    double new_mean;
 ...
 */
void RV::estimateFullCovarianceMatrixML(int population_index) {
    int i, j, m;
    double cov;
...
void RV::estimateCovarianceMatricesML(int population_index) {
...
void RV::initializeCovarianceMatrices(int population_index) {
    int j, k, m;

    decomposed_covariance_matrices[population_index] = (double ***) Malloc(
            linkage_model[population_index]->length * sizeof(double **));
    for (j = 0; j < linkage_model[population_index]->length; j++) {
        decomposed_covariance_matrices[population_index][j] = (double **) Malloc(
                linkage_model[population_index]->set_length[j] * sizeof(double *));
        for (k = 0; k < linkage_model[population_index]->set_length[j]; k++) {
            decomposed_covariance_matrices[population_index][j][k] = (double *) Malloc(
                    linkage_model[population_index]->set_length[j] * sizeof(double));
...
void RV::copyBestSolutionsToAllPopulations() {
    int i;
 ...
 */
void RV::copyBestSolutionsToPopulation(int population_index) {
    int k;
...
void RV::getBestInPopulation(int population_index, int *individual_index) {
    int i;
...
void RV::getOverallBest(int *population_index, int *individual_index) {
    ...
    *individual_index = best_individual_index;
    for (i = 0; i < number_of_populations; i++) {
        ...
        if (betterFitness(objective_values[i][best_individual_index], constraint_values[i][best_individual_index],
                          objective_values[*population_index][*individual_index],
                          constraint_values[*population_index][*individual_index])) {
            *population_index = i;
            *individual_index = best_individual_index;
...
void RV::evaluateCompletePopulation(int population_index) {
    int j;
    for (j = 0; j < population_sizes[population_index]; j++)
        installedProblemEvaluation(problem_index, populations[population_index][j],
                                   &(objective_values[population_index][j]), &(constraint_values[population_index][j]),
 ...
 */
void RV::applyDistributionMultipliersToAllPopulations() {
    int i;
...
void RV::applyDistributionMultipliers(int population_index) {
    int j, k, m;
...
void RV::generateAndEvaluateNewSolutionsToFillAllPopulations() {
    int i;
 ...
 */
void RV::generateAndEvaluateNewSolutionsToFillPopulation(int population_index) {
    ...
    if (!populations_terminated[population_index]) {
        ...
        alpha_AMS = 0.5 * tau * (((double) population_sizes[population_index]) /
                                 ((double) (population_sizes[population_index] - 1)));
        ...
        for (oj = 0; oj < linkage_model[population_index]->length; oj++) {
            ...
            apply_AMS = 1;
            for (k = 1; k < population_sizes[population_index]; k++) {
                short apply_ams_copy = k > number_of_AMS_solutions ? 0 : 1;
//                if( k > number_of_AMS_solutions ) apply_AMS = 0;
                individual_improved[k] |= generateNewSolutionFromFOSElement(population_index, j, k, apply_ams_copy);
            }
 ...
 */
void RV::computeParametersForSampling(int population_index) {
    int i;

    if (!use_univariate_FOS) {
        decomposed_cholesky_factors_lower_triangle[population_index] = (double ***) Malloc(
                linkage_model[population_index]->length * sizeof(double **));
        for (i = 0; i < linkage_model[population_index]->length; i++)
 ...
 */
double *RV::generateNewPartialSolutionFromFOSElement(int population_index, int FOS_index) {
        ...
        if (times_not_in_bounds >= 100) {
            ...
            if (use_univariate_FOS) {
                ...
                result = matrixVectorMultiplication(
                        decomposed_cholesky_factors_lower_triangle[population_index][FOS_index], z, num_indices,
                        num_indices);
                for (i = 0; i < num_indices; i++)
 ...
 */
short
RV::generateNewSolutionFromFOSElement(int population_index, int FOS_index, int individual_index, short apply_AMS) {
    ...
    installedProblemEvaluation(problem_index, populations[population_index][individual_index], &obj_val, &cons_val,
                               num_touched_indices, touched_indices, individual_backup,
                               objective_values[population_index][individual_index],
                               constraint_values[population_index][individual_index]);
    improvement = betterFitness(obj_val, cons_val, objective_values[population_index][individual_index],
                                constraint_values[population_index][individual_index]);
    if (improvement) {
        any_improvement = 1;
...
short RV::applyAMS(int population_index, int individual_index) {
    ...
    if (!out_of_range) {
        installedProblemEvaluation(problem_index, solution_AMS, &obj_val, &cons_val, number_of_parameters, NULL, NULL,
                                   0, 0);
        if (randomRealUniform01() < 0.05 ||
            betterFitness(obj_val, cons_val, objective_values[population_index][individual_index],
                          constraint_values[population_index][individual_index])) {
            objective_values[population_index][individual_index] = obj_val;
            constraint_values[population_index][individual_index] = cons_val;
            for (m = 0; m < number_of_parameters; m++)
...
void RV::applyForcedImprovements(int population_index, int individual_index, int donor_index) {
    ...
    while (alpha >= 0.01) {
        ...
        for (io = 0; io < linkage_model[population_index]->length; io++) {
            ...
            installedProblemEvaluation(problem_index, populations[population_index][individual_index], &obj_val,
                                       &cons_val, num_touched_indices, touched_indices, FI_backup,
                                       objective_values[population_index][individual_index],
                                       constraint_values[population_index][individual_index]);
            improvement = betterFitness(obj_val, cons_val, objective_values[population_index][individual_index],
                                        constraint_values[population_index][individual_index]);
 ...
 */
short RV::adaptDistributionMultipliers(int population_index, int FOS_index) {
 ...
 */
short
RV::generationalImprovementForOnePopulationForFOSElement(int population_index, int FOS_index, double *st_dev_ratio) {
 ...
 */
double RV::getStDevRatioForFOSElement(int population_index, double *parameters, int FOS_index) {
 ...
 * Undoes initialization procedure by freeing up memory.
 */
void RV::ezilaitiniDistributionMultipliers(int population_index) {
    free(distribution_multipliers[population_index]);
    free(samples_drawn_from_normal[population_index]);
    free(out_of_bounds_draws[population_index]);
}

void RV::ezilaitiniCovarianceMatrices(int population_index) {
    int i, j, k;
...
void RV::ezilaitiniParametersAllPopulations() {
    int i;
 ...
 */
void RV::ezilaitiniParametersForSampling(int population_index) {
    int i, j;
...
void RV::generationalStepAllPopulations() {
...
void RV::generationalStepAllPopulationsRecursiveFold(int population_index_smallest, int population_index_biggest) {
    int i, j, population_index;

    for (i = 0; i < number_of_subgenerations_per_population_factor - 1; i++) {
        for (population_index = population_index_smallest;
...
void RV::runAllPopulations() {
    while (!checkTerminationCondition()) {
        ...
        elitist_objective_values.push_back(elitist_objective_value);
...
double RV::run() {
    ...
    runAllPopulations();
    int population_index_best, individual_index_best;
    determineBestSolutionInCurrentPopulations(&population_index_best, &individual_index_best);
    for (size_t j = 0; j < FCs.size(); j++) {

        ((OpFunctionClassConst *)(FCs[j]))->SetConstant(
                populations[population_index_best][individual_index_best][j]);
    }

//    std::string s =  "params: " + std::to_string(number_of_parameters) + ", time: " + std::to_string(getTimer()) + ", evals: " + std::to_string(number_of_evaluations) + "\n";
//    std::cout << s;
    if (std::isnan(elitist_objective_value)) {
        elitist_objective_value = arma::datum::inf;
    }
    if (!get_solutions){
        ezilaitini();
    }
    return elitist_objective_value;

}

void RV::saveSolutions() {

    int population_index_best, individual_index_best;
    determineBestSolutionInCurrentPopulations(&population_index_best, &individual_index_best);

/* Then write them */
    std::string plotpath = path;
    std::ofstream myfile(plotpath, std::ios::app);
    myfile << tree_index << ",";
    myfile << tree_nr << ",";
    myfile << data_index << ",";
    for (int i = 0; i < number_of_parameters; i++) {
        myfile << populations[population_index_best][individual_index_best][i] << ",";
    }
    myfile << std::endl;
    ezilaitini();
}
...
RV::RV(Node *tree, std::vector<Operator *> FCs, arma::mat x, arma::mat y_p, bool get_solutions, size_t nr_per_set, size_t evals, std::string path,  size_t tree_index, size_t tree_nr, size_t data_index, bool calc_constraints) : evals(evals), nr_per_set(nr_per_set), tree(tree), FCs(std::move(FCs)),
                                                                                            x(std::move(x)), y(std::move(y_p)), path(path), get_solutions(get_solutions), tree_index(tree_index), tree_nr(tree_nr), data_index(data_index), calc_constraints(calc_constraints) {
}
