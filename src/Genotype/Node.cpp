...
void Node::ClearCachedOutput(bool also_ancestors, bool also_children){
...
std::string Node::GetSubtreeExpression(bool only_active, bool shadow, bool spaces){
    throw NotImplementedException("Node::GetSubtreeExpression not implemented.");
}

std::string Node::GetSubtreeHumanExpression(bool shadow){
    throw NotImplementedException("Node::GetSubtreeHumanExpression not implemented.");
}

std::string Node::GetPythonExpression(bool shadow){
...
int Node::DominatesTernary(Node *o) {
    bool strictly_better_somewhere = false;
    if (this->constraints_violated < o->constraints_violated ){
        return 1;
    }
    if (this->constraints_violated > o->constraints_violated  ){
        return -1;
...
bool Node::Dominates(Node * o) {
    return DominatesTernary(o) > 0;
}
int Node::Count_N_NaComp(int count) {
    throw NotImplementedException("Node::Count_N_NaComp not implemented.");
}

std::vector<Operator *> Node::GetNrFCConstants() {
    throw NotImplementedException("Node::GetNrFCConstants not implemented.");

}

size_t Node::GetNrSingleNodes() {
    throw NotImplementedException("Node::GetNrSingleNodes not implemented.");

}

Node * Node::CloneSingleSubtree(size_t index) {
    throw NotImplementedException("Node::GetNrSingleNodes not implemented.");
