...
void Multitree::ClearCachedOutput(bool also_ancestors, bool also_children) {
    cached_output.clear();
}
...
std::string Multitree::GetSubtreeExpression(bool only_active_nodes, bool shadow, bool spaces) {
    string expr;
    size_t index = 1;
    for (SingleNode * n : nodes){
        expr += "Expression" + std::to_string(index);
        expr += n->GetSubtreeExpression(only_active_nodes, shadow, spaces);
        expr += "\n ";
        index++;
    }
    return expr;
...
std::string Multitree::GetSubtreeHumanExpression(bool shadow) {
    string expr;
    size_t index = 1;
    for (SingleNode * n : nodes){
        expr += "Expression " + std::to_string(index) + ": ";
        expr += n->GetSubtreeHumanExpression(shadow);
//        expr += "\n ";
        index++;
...
std::string Multitree::GetPythonExpression(bool shadow) {
    string expr;
    size_t index = 1;
    for (SingleNode * n : nodes){
        expr += "Expression " + std::to_string(index) + ": ";
        expr += n->GetPythonExpression(shadow);
//        expr += "\n ";
        index++;
...
std::vector<Operator *> Multitree::GetNrFCConstants() {
    std::vector<Operator *> FCC;
    for (SingleNode * n: nodes) {
        auto node_fcs = n->GetNrFCConstants();
        FCC.insert(FCC.end(), node_fcs.begin(), node_fcs.end());
    }
    return FCC;
}

size_t Multitree::GetNrSingleNodes() {
    return this->nodes.size();
}

Node * Multitree::CloneSingleSubtree(size_t index){
    Node * new_node = new Multitree();
    for(size_t i = 0; i<nodes.size(); i++){
        new_node->AppendSingleNode(nodes[index]->CloneSubtree());
    }
    return new_node;
}