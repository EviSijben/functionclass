...
arma::vec Fitness::GetPopulationTestFitness(const std::vector<Node*>& population) {

    vec fitnesses(population.size());
#pragma omp parallel for schedule(dynamic)
        for (size_t i = 0; i < population.size(); i++) {
            fitnesses[i] = GetTestFit(population[i]);
        }
    return fitnesses;
}
...
Node * Fitness::GetBestWithConstraints(const std::vector<Node*>& population, bool compute, bool use_caching) {

    Node * best;
    double best_fitness;
    size_t best_constraints ;
    bool init = false;

    if (compute) {
        for (size_t i = 0; i < population.size(); i++) {
            ComputeFitness(population[i], use_caching);
            if (!init || population[i]->constraints_violated < best_constraints|| (population[i]->cached_fitness <= best_fitness && population[i]->constraints_violated == best_constraints) ) {
                best = population[i];
                best_fitness = population[i]->cached_fitness;
                best_constraints = population[i]->constraints_violated;
                init = true;
