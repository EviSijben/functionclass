...
std::vector<std::vector<size_t> > GOMEAFOS::GenerateFOS(const std::vector<Node*>& population, FOSType fos_type, bool fos_no_root_swap, arma::mat * MI_distribution_adjustments) {
    ...
    if (fos_type == FOSType::FOSLinkageTree)
        FOS = GenerateLinkageTreeFOS(number_of_nodes, population, MI_distribution_adjustments, fos_no_root_swap,false, true);
    else if (fos_type == FOSType::FOSLinkageTreeIdv)
        FOS = GenerateLinkageTreeFOS(number_of_nodes, population, MI_distribution_adjustments, fos_no_root_swap,true, true);
    else if (fos_type == FOSType::FOSLinkageTreeIdvO)
        FOS = GenerateLinkageTreeFOS(number_of_nodes, population, MI_distribution_adjustments, fos_no_root_swap,true, false);
...
std::vector<std::vector<size_t> > GOMEAFOS::GenerateLinkageTreeFOS(size_t number_of_nodes, const std::vector<Node*>& population, arma::mat * MI_distribution_adjustments, bool fos_no_root_swap, bool idv_trees, bool multi_tree_fos) {
    ...
    for (size_t i = 0; i < pop_size; i++) {
        ...
        for (size_t j = 0; j < number_of_nodes; j++) {
            ...
            if (n->GetOperator()->type == OperatorType::opTermConstant) {
                if (constants.size() < max_constants) {
                    constants.insert(((OpRegrConstant*) n->GetOperator())->GetConstant());
                    ...
                    const double_t q = ((OpRegrConstant*) n->GetOperator())->GetConstant();
    ...
    // assemble the Linkage Tree with UPGMA for each tree in multitree if it is a multitree
    if (population[0]->type == NodeType::Multi ){
        size_t nr_trees = ((Multitree *) population[0])->nodes.size();
        size_t nr_nodes_pt = number_of_nodes/nr_trees;
        vector<vector < double_t >> mi_matrix_t(nr_nodes_pt, vector<double_t>(nr_nodes_pt, 0.0));
        for (size_t nr_tree =0; nr_tree<nr_trees;nr_tree++){
            // slices
            for (size_t x1 =0; x1<nr_nodes_pt; x1++){
                for (size_t y1 =0; y1<nr_nodes_pt; y1++){
                    mi_matrix_t[x1][y1] = mi_matrix[x1+nr_tree*nr_nodes_pt][y1+nr_tree*nr_nodes_pt];
                }
            }
            auto tree_fos = BuildLinkageTreeFromSimilarityMatrix(nr_nodes_pt, mi_matrix_t);
//tree_fos.pop_back(); // remove the root of the Linkage Tree (otherwise entire solution can be swapped during GOM)
            auto idx_offset = nr_tree * nr_nodes_pt;
            for (auto &vec : tree_fos) {
                for (auto &val : vec) {
                    val += idx_offset;
                }
            }
            if (idv_trees)
                FOS.insert(FOS.end(), tree_fos.begin(), tree_fos.end());
            else FOS.push_back(tree_fos.back());
        }
    }

    // assemble the Linkage Tree with UPGMA
    if (multi_tree_fos || !(population[0]->type == NodeType::Multi) ){
        auto total_fos = BuildLinkageTreeFromSimilarityMatrix(number_of_nodes, mi_matrix);
        total_fos.pop_back(); // remove the root of the Linkage Tree (otherwise entire solution can be swapped during GOM)
        FOS.insert(FOS.end(), total_fos.begin(), total_fos.end());
    }
    ...
    if (fos_no_root_swap) {
        for (size_t i = 0; i < FOS.size(); i++) {
            ...
            if (population[0]->type == NodeType::Multi) {
                size_t nr_trees = ((Multitree *) population[0])->nodes.size();
                size_t nr_nodes_pt = number_of_nodes/nr_trees;
                for (size_t root = 0; root < number_of_nodes; root += nr_nodes_pt) {
                    auto it = find(set->begin(), set->end(), root);
                    if (it != set->end()) {
                        set->erase(it);
                    }
                }
