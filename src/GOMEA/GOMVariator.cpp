...
Node *GOMVariator::GOMMO(const Node &sol,const std::vector<Operator *> &functions, const std::vector<Operator *> &terminals, const std::vector<std::vector<size_t> > &FOS,
                         ...
                         size_t NIS_const, bool batching) {
    Node *offspring = sol.CloneSubtree();
    size_t back_cons = sol.constraints_violated;
    ...
    for (size_t i = 0; i < FOS.size(); i++) {
        ...
        if (meaningful_change) {
            ...
            std::pair<bool, bool> check_changes = extreem ? check_changes_SO(offspring, back_obj, back_cons, false, obj)
                                    : check_changes_MO(offspring, back_obj, back_cons, false, mo_archive, batching);
            if(check_changes.second){
                improved = true;
            }
            if (check_changes.first) {
                // keep changes
                back_cons = offspring->constraints_violated;
                back_obj = offspring->cached_objectives;
                changed = true;
                mo_archive->UpdateMOArchive(offspring, batching);
                mo_archive->UpdateSOArchive(offspring, batching);
            } else {
                // revert
                RevertChanges(changed_indices, offspring_nodes, backup_operators, use_caching);
                offspring->cached_fitness = back_obj[0];
                offspring->constraints_violated = back_cons;
    ...
    if ((!changed || offspring->NIS > NIS_const) && mo_archive->IsNotEmpty(batching)) {
        offspring->NIS = 0;
        return GOMVariator::GOMMOFI(offspring,  FOS, fit, use_caching, mo_archive, obj, extreem, NIS_const, batching);
    }
...
Node *GOMVariator::GOMMOFI(Node *& offspring, const std::vector<std::vector<size_t> > &FOS,
    ...
    size_t back_cons = offspring->constraints_violated;
    ...
    for (size_t i = 0; i < FOS.size(); i++) {
        vector<size_t> F = FOS[permut_fos_indices[i]];
        auto donor = extreem ? mo_archive->ReturnCopySOMember(obj, batching) : mo_archive->ReturnCopyRandomMOMember(batching);
        ...
        if (meaningful_change) {
            fit.ComputeFitness(offspring, use_caching);
            std::pair<bool, bool> check_changes = extreem ? check_changes_SO(offspring, back_obj, back_cons, true, obj)
                                                          : check_changes_MO(offspring, back_obj, back_cons, true, mo_archive,batching);
            if (check_changes.first) {
                // keep changes
                back_obj = offspring->cached_objectives;
                back_cons = offspring->constraints_violated;
    ...
    if ((!changed ) && mo_archive->IsNotEmpty(false)) {
        offspring->ClearSubtree();
        if (extreem)
            return mo_archive->ReturnCopySOMember(obj, false);
        else return mo_archive->ReturnCopyRandomMOMember(false);
    }
    return offspring;
}

std::pair<bool, bool>
GOMVariator::check_changes_SO(Node *offspring, arma::vec back_obj, size_t back_cons, bool FI, size_t obj) {
    if (offspring->constraints_violated < back_cons) {
        return std::make_pair(true,true);

    }
    if (offspring->cached_objectives[obj] < back_obj[obj] && offspring->constraints_violated == back_cons) {
        return std::make_pair(true,true);
    }
    if (offspring->cached_objectives[obj] == back_obj[obj] && offspring->constraints_violated == back_cons) {
        if (FI) {
...
std::pair<bool, bool>
GOMVariator::check_changes_MO(Node *offspring, arma::vec back_obj, size_t back_cons , bool FI,
                              MOArchive *mo_archive, bool batching) {
    ...
    if (dominates && offspring->constraints_violated == back_cons) {
        return std::make_pair(true, true);
    }
    ...
    if (!FI) {
        ...
        if (same && offspring->constraints_violated == back_cons)
            return std::make_pair(true,false);
    }

    // or is not dominated by mo_archive
    if (mo_archive->NonDominated(offspring,batching)) {
        mo_archive->UpdateMOArchive(offspring, batching);
        return std::make_pair(true,true);
...
Node *GOMVariator::GOM(const Node &sol, const std::vector<Node *> &donors, const std::vector<std::vector<size_t>> &FOS,
    ...
    for (size_t i = 0; i < FOS.size(); i++) {
        ...
        if (meaningful_change) {
            ...
            keep_changes = offspring->constraints_violated < back_cons ||
                           (new_fit <= back_fit && offspring->constraints_violated == back_cons);
        }
        if (keep_changes) {
...
GOMVariator::GOM(const Node &sol, const std::vector<Operator *> &functions, const std::vector<Operator *> &terminals,
    ...
    for (size_t i = 0; i < FOS.size(); i++) {
        ...
        //if we use probabilistic acceptance, then we flip the decision in 10 percent of cases
        if (keep_changes) {
            // keep changes
            back_fit = offspring->cached_fitness;
            back_cons = offspring->constraints_violated;
...
//                } else {
//                    // this is not yet implemented for unbalanced template
