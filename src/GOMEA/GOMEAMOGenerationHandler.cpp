...
//
    std::sort(idx.begin(), idx.end(), [&](const std::pair<size_t, size_t> &lhs, const std::pair<size_t, size_t> &rhs) {
        auto &lhs_node = clustered_population[lhs.first][lhs.second];
        auto &rhs_node = clustered_population[rhs.first][rhs.second];
        return lhs_node->GetNrFCConstants().size() > rhs_node->GetNrFCConstants().size();
    });
    ...
    for (size_t x = 0; x <idx.size(); x++) {
            ...
            if (clustered_population[i].size() > 1) {
                off = GOMVariator::GOMMO(clustered_population[i][j], clustered_population_equal[i], FOSs[i], *fitness,
                                         conf->caching, mo_archive, clusternr[i], clusternr[i] < nr_obj, NIS, conf->batch_size> 0);
            } else{
...
bool GOMEAMOGenerationHandler::CheckPopulationConverged(const std::vector<Node *> &population) {
    if (mo_archive->LastGenChanged()>3){
        return true;
    }
    return false;
...
GOMEAMOGenerationHandler::K_leader_means(std::vector<Node *> &population) {
    ...
    size_t random_obj = std::round(arma::randu())*nr_objs;
...
// k-means
    ...
    while (cluster_change && iter < 50) {
        ...
        for (size_t i = 0; i < population.size(); i++) {
            ...
            // cluster_centers_temp are updated as well as the number of individuals beloning to a center, for calculating the new clustercenters later on
